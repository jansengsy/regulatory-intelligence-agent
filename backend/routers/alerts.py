...
def list_alerts(
    ...
    analyzed: bool | None = Query(None, description="Filter by analysis status"),
    ...
    if analyzed is not None:
        query = query.where(Alert.analyzed == analyzed)
...
def alert_stats(session: Session = Depends(get_session)):
    ...
    analyzed_count = session.exec(
        select(func.count(Alert.id)).where(Alert.analyzed == True)  # noqa: E712
    ).one()
    ...
    # Counts grouped by severity (only meaningful once alerts are analyzed)
